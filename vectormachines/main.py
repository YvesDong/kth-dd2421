...
def generate_data(Num, pos1=[-1.5, 0.5]):
    ...
    # np.random.seed(100)
    classA = np.concatenate((np.random.randn(int(Num/2), 2)*0.2 + [1.5, 0.5], np.random.randn(int(Num/2), 2)*0.2 + pos1))
...
def plot_results(axs, kernel="linKernel", p=2, sigma=1):
    ...
    axs.set_title("# sigma RBFK: {}, % pos alpha: {}".format(sigma, "%.2f" % (len(supportvectors)/(2*Num))))
